...
namespace EventSourcingAndCQRS
{
    ...
    internal class PrivateReflectionDynamicObject : DynamicObject
    {
        ...
        {
            var isMissingMethodExceptionLaunched = false;
            result = InvokeMemberOnType(RealObject.GetType(), RealObject, binder.Name, args, out isMissingMethodExceptionLaunched);
            if (isMissingMethodExceptionLaunched)
            {
                throw new MissingMethodException(RealObject.GetType().FullName, binder.Name);
            }
        ...
        private static object InvokeMemberOnType(Type type, object target, string name, object[] args, out bool isMissingMethodExceptionLaunched)
        {
            isMissingMethodExceptionLaunched = false;
            try
            {
            ...
            {
                isMissingMethodExceptionLaunched = true;
                // If we couldn't find the method, try on the base class
                if (type.BaseType != null)
                {
                    return InvokeMemberOnType(type.BaseType, target, name, args, out isMissingMethodExceptionLaunched);
                }
